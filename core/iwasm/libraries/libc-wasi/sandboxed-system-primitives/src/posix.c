...
/*
 * convert_clockid is a plceholder in this file so that poll_oneoff complies,
 * the implementation has been refactored to posix_clock.c. In the near future
 * poll_oneoff will be refactored and the below placeholder will be removed.
 */
bool
...
convert_stat(const struct stat *in, __wasi_filestat_t *out)
{
    *out = (__wasi_filestat_t){
        ...
        .st_atim = convert_timespec_to_nanoseconds(&in->st_atim),
        .st_mtim = convert_timespec_to_nanoseconds(&in->st_mtim),
        .st_ctim = convert_timespec_to_nanoseconds(&in->st_ctim),
    };
}
#endif
